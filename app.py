...
def calculate_complex_impacts(lat, lon, s_lat, s_lon, p):
    ...
    # Asymmetry (Shear Tilt)
    shear_rad = np.radians(shear_dir)
    shear_effect = 1 + (shear_mag / 60) * np.cos(angle - shear_rad)
    
    total_wind = (v_sym * shear_effect) + v_forward
    if lat > 30.25: total_wind *= 0.85 # Land friction for Mobile County
    
    # 3. RAINFALL (R-CLIPER + Shear Tilt)
    # Heaviest rain shifted downshear-left
    ...
    # 4. SIMULATED IR SATELLITE (Cloud Top Brightness)
    # CDO canopy offset by shear
    off_x, off_y = (shear_mag/8) * np.cos(shear_rad), (shear_mag/8) * np.sin(shear_rad)
    r_sat = np.sqrt((dx - off_x)**2 + (dy - off_y)**2)
    brightness = 255 * np.exp(-r_sat / (r_max * 5)) * (rh / 100)
    
    return max(0, total_wind), total_rain, min(255, brightness)

def calculate_bay_surge(v_max, s_lon):
    """Calculates surge specifically for the Mobile Bay funnel"""
    # Storm west of Bay = South winds = High Surge
...
st.markdown("""
    ...
    <div class="legend">
        ...
        <hr>
        <b>IR Layer:</b> Brighter = Higher Tops
    </div>
...
# Generate Grid
grid_n = 22 if show_ir else 30
lats = np.linspace(29.8, 31.3, grid_n)
lons = np.linspace(-88.8, -87.4, grid_n)
results = []

for lt in lats:
    for ln in lons:
        w, rain, ir = calculate_complex_impacts(lt, ln, l_lat, l_lon, p)
        results.append([lt, ln, w, rain, ir])

df = pd.DataFrame(results, columns=['lat', 'lon', 'wind', 'rain', 'ir'])
...
with c1:
    ...
    for _, row in df.iterrows():
        # IR Satellite Layer (White Heatmap)
        if show_ir and row['ir'] > 40:
            folium.Circle(
                location=[row['lat'], row['lon']],
                radius=5000,
                color=None, fill=True, fill_color='white',
                fill_opacity=row['ir']/600
            ).add_to(m)
        ...
        if row['wind'] > 34:
            ...
            folium.CircleMarker(
                ...
                popup=f"Wind: {int(row['wind'])} kts\nRain: {row['rain']:.1f} in/hr"
            ).add_to(m)
...
with c2:
    ...
    if v_max < 70 and shear_mag > 25:
        st.warning("⚠️ Disorganized 'Halfacane' detected. Expect highly asymmetric impacts.")
    if l_lon < -88.2:
        st.error("⚠️ DIRTY SIDE: Mobile Bay is in the right-front quadrant. High surge risk.")
